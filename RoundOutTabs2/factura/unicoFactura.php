...
//Busqueda y localización de la factura
...
$item->codigoFac = isset($_GET['codigoFac']) ? $_GET['codigoFac'] : die();
$item->getSingleFactura();
if ($item->codigoFac != null) {

// create array
    $fac_arr = array(
            "idFac" => $item->idFac,
        "codigoFac" => $item->codigoFac,
    ...
    $myjson = json_encode($fac_arr);

    //Busqueda y localización de Artículos
    for ($i=1; $i<=10; $i++) {
        $busqueda = $fac_arr['item'.$i];
        if ($busqueda != "") {
...
// create array
    ...
    echo "<br>";
    echo '<h3>Información del Albarán: ' . '</h3>';
    echo "<br>";
    echo "-------------------------------------------------------------------------<br>";
    echo "<div id='resultado' onclick='setData(this)'>";
    echo "-ID Factura: -".$item->idFac;
    echo "<br>";
    echo "-Código: -" .$item->codigoFac;
    echo "<br>";
    ...
    echo json_encode("Factura no encontrada.");
}
?>
<div>
    <script type="text/javascript">
        function setData(sel) {
            ...
            if (window.opener != null && !window.opener.closed) {
                ...
                window.opener.document.getElementById('codigoFacBorrar').value = idFac;
                window.opener.document.getElementById('codigo').value = codigoFac;
