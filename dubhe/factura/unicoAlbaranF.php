...
include('../albaran/conexionAlbaranes.php');
include('../albaran/gestionAlbaranes.php');
$database = new DatabaseA();
$dbA = $database->getConnection();
$item = new Albaran($dbA);
$item->codigoAlb = isset($_GET['codigoAlb']) ? $_GET['codigoAlb'] : die();
$item->getSingleAlbaran();
if ($item->codigoAlb != null) {

// create array
    $alb_arr = array(
            "idAlb" => $item->idAlb,
        "codigoAlb" => $item->codigoAlb,
    ...
    $myjson = json_encode($alb_arr);

    //Busqueda y localización de Artículos
    for ($i=1; $i<=10; $i++) {
        $busqueda = $alb_arr['item'.$i];
        if ($busqueda != "") {
...
// create array
    ...
    echo "-ID Albarán: -".$item->idAlb;
    echo "<br>";
    echo "-Código: -" .$item->codigoAlb;
    echo "<br>";
    ...
    echo "<br>";
    echo json_encode("Albaran no encontrado.");
}
?>
<div>
...
<br><br><a class="center" href="buscarAlbaranF.php" style="font-weight: bold">Volver</a>
</body>
